...
private:
    ...
    cond_locker queue_cond_locker; //条件变量,当新增task后，通知从task queue中取任务并执行
    bool is_stop; //是否结束线程
public:
...
void threadpool<T>::start()
{
    ...
    {
	    //printGreen("create the %dth pthread\n", i);
...
{   
	printGreen("now append a new task")
	//获取互斥锁
...
void *threadpool<T>::worker(void *arg)
{
	...
	printGreen("come in thread func worker")
    pool->run();
    return pool;
}
...
{
	printGreen("come in run")
    while(!is_stop){
        T *task = getTask();
        if(task == NULL)
        {
        	printGreen("before wait");
        	queue_cond_locker.wait();
			printGreen("wait get signal");
		}
        else
    	{
    		printGreen("before task->doit()")
            task->doit();
    		delete task;
    	}
    }
